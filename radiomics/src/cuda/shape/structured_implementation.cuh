#ifndef SHAPE_STRUCTURED_IMPLEMENTATION_CUH_
#define SHAPE_STRUCTURED_IMPLEMENTATION_CUH_

#include "tables.cuh"
#include "../vertex_structure.cuh"
#include <cstddef>

static __global__ void calculate_coefficients_structured_kernel(
    const char *mask, const int *size, const int *strides,
    const double *spacing, double *surfaceArea, double *volume,
    VertexStructure vertices, unsigned long long *vertex_count, size_t max_vertices) {
    ...
    double new_vertices_x[3]; // Max 3 vertices
    double new_vertices_y[3]; // Max 3 vertices
    double new_vertices_z[3]; // Max 3 vertices

    // Check bit 6 (original point p6, edge 6) using potentially flipped cube_idx
    if (cube_idx & (1 << 6)) {
        int edge_idx = 6;
        new_vertices_x[num_new_vertices] = (((double) iz) + d_vertList[edge_idx][0]) * spacing[0];
        new_vertices_y[num_new_vertices] = (((double) iy) + d_vertList[edge_idx][1]) * spacing[1];
        new_vertices_z[num_new_vertices] = (((double) ix) + d_vertList[edge_idx][2]) * spacing[2];
        num_new_vertices++;
    }
    ...
    if (num_new_vertices > 0) {
        unsigned long long start_v_idx = atomicAdd(vertex_count, (unsigned long long) num_new_vertices);
        ...
        if (start_v_idx + num_new_vertices <= max_vertices) {
            for (int v = 0; v < num_new_vertices; ++v) {
                ...
                vertices.x[write_idx] = new_vertices_x[v];
                vertices.y[write_idx] = new_vertices_y[v];
                vertices.z[write_idx] = new_vertices_z[v];
            }
        }
    ...
    while (d_triTable[cube_idx][t * 3] >= 0) {
        ...
        for (int d = 0; d < 3; ++d) {
            p1[d] = (((double) (d == 0 ? iz : (d == 1 ? iy : ix))) + d_vertList[v_idx_1][d]) * spacing[d];
            p2[d] = (((double) (d == 0 ? iz : (d == 1 ? iy : ix))) + d_vertList[v_idx_2][d]) * spacing[d];
            p3[d] = (((double) (d == 0 ? iz : (d == 1 ? iy : ix))) + d_vertList[v_idx_3][d]) * spacing[d];
        }
...
#endif // SHAPE_STRUCTURED_IMPLEMENTATION_CUH_ 